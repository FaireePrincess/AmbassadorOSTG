...
async function ensureUserOwnsTweet(userId: string, authorHandle: string): Promise<void> {
  ...
  if (profileHandle !== authorHandle.toLowerCase()) {
...
async function ensureTweetIsGloballyUnique(tweetId: string): Promise<void> {
  ...
  const duplicateExtra = extraContent.find((item) => item.tweetId === tweetId);
  if (duplicateExtra) {
    throw new Error("This X post has already been submitted");
  }

  const duplicateTaskSubmission = submissions.find((submission) => {
    const urls = [
      submission.postUrl,
...
export const extraContentRouter = createTRPCRouter({
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const extraContent = await getExtraContent();
      const existing = extraContent.find((item) => item.id === input.id);
      if (!existing) throw new Error("Extra content not found");
      if (input.userId && existing.userId !== input.userId) {
        throw new Error("You can only delete your own extra content");
      }
      await db.remove(EXTRA_CONTENT_COLLECTION, input.id);
      return { success: true };
    }),
});
